...
if (isset($_GET['id']) && isset($_GET['aksi']) && $_GET['aksi'] == 'ubah') {
    $id = $_GET['id'];
    $result = mysqli_query($conn, "SELECT * FROM periksa WHERE id='$id'");
...
<div class="container mt-4">
    ...
    <table class="table table-hover mt-4">
        ...
        <tbody>
            <?php
            $result = mysqli_query($conn, "SELECT pr.*, d.nama as 'nama_dokter', p.nama as 'nama_pasien' FROM periksa pr LEFT JOIN dokter d ON pr.id_dokter = d.id LEFT JOIN pasien p ON pr.id_pasien = p.id ORDER BY pr.tgl_periksa DESC");
            $no = 1;
    ...
    if (isset($_POST['simpan'])) {
        ...
        $tambah = mysqli_query($conn, "INSERT INTO periksa (id_pasien, id_dokter, tgl_periksa, catatan, obat) VALUES ('$id_pasien', '$id_dokter', '$tgl_periksa', '$catatan', '$obat')");
    ...
    if (isset($_POST['update'])) {
        ...
        $update = mysqli_query($conn, "UPDATE periksa SET id_pasien='$id_pasien', id_dokter='$id_dokter', tgl_periksa='$tgl_periksa', catatan='$catatan', obat='$obat' WHERE id='$id'");
        echo "<script>document.location='index.php?page=periksa';</script>";
    }
    ...
    if (isset($_GET['aksi']) && $_GET['aksi'] == 'hapus') {
        $id = $_GET['id'];
        $hapus = mysqli_query($conn, "DELETE FROM periksa WHERE id='$id'");
        echo "<script>document.location='index.php?page=periksa';</script>";
    }
    ?>
</div>
